...
async function getFixtureOdds(request: Request) {
  ...
  result.rows.forEach(row => {
    ...
    if (row.odds_x12 || row.odds_ah || row.odds_ou || row.fair_odds_x12 || row.fair_odds_ah || row.fair_odds_ou) {
      ...
      // If latest=true, just take the last entry (even if it's zero odds)
      if (useLatest) {
        if (oddsX12 && oddsX12.length > 0) oddsX12 = [oddsX12[oddsX12.length - 1]];
        if (oddsAh && oddsAh.length > 0) oddsAh = [oddsAh[oddsAh.length - 1]];
        if (oddsOu && oddsOu.length > 0) oddsOu = [oddsOu[oddsOu.length - 1]];
        if (lines && lines.length > 0) lines = [lines[lines.length - 1]];
      }
