...
import { poissonPMF, poissonProbabilities, dixonColesAdjustment, poissonAsianHandicapProbabilities } from './poisson-utils';

interface AsianHandicapResult {
  homeWinProb: number;
  awayWinProb: number;
}
...
function calculateAsianHandicap(
  ...
  if (isQuarterLine) {
    ...
    let lowerLine: number, upperLine: number;
...
function calculateOverUnder(
  ...
  if (userRho !== 0) {
    const adjustment = Math.exp(userRho * (homeXg + awayXg));
    overProb *= adjustment;
    underProb *= adjustment;
  }
...
/**
 * Calculate and populate prediction odds from MLP model predictions
 * Converts home_pred and away_pred to betting odds format
    ...
    for (let i = 0; i < predictions.length; i += batchSize) {
      ...
      for (const prediction of batch) {
        try {
          ...
          const ahLines: number[] = [];
          const ahOddsHome: number[] = [];
          const ahOddsAway: number[] = [];
          ...
          if (existingResult.rows.length > 0) {
            ...
            if (existing.odds_x12) {
              ...
              const existingIndex = existingX12.findIndex((entry: any) => entry.t === timestamp);
              if (existingIndex >= 0) {
            ...
            if (existing.odds_ou) {
              ...
              const existingIndex = existingOu.findIndex((entry: any) => entry.t === timestamp);
              if (existingIndex >= 0) {
            ...
            if (existing.odds_ah) {
              ...
              const existingIndex = existingAh.findIndex((entry: any) => entry.t === timestamp);
              if (existingIndex >= 0) {
            ...
            if (existing.lines) {
              ...
              const existingIndex = existingLines.findIndex((entry: any) => entry.t === timestamp);
              if (existingIndex >= 0) {
