...
export default function DataTable<T>({
  ...
  return (
    <div className={`space-y-1 ${className}`} style={{ pointerEvents: 'none' }}>
      {/* Header */}
      <div className={`flex justify-between items-center py-2 border-b border-gray-800 ${headerClassName}`}>
      ...
      {/* Column Headers */}
      <div ref={dropdownRef} className={`relative flex gap-1 py-1 bg-gray-900/80 backdrop-blur-sm border-b border-gray-800 text-[10px] font-bold text-gray-400 uppercase tracking-wider`}>
        {selectable && (
      ...
      <div className="space-y-0">
        {displayData.length === 0 ? (
          ...
          displayData.map((item, index) => {
            ...
            const RowContent = (
              <div
                className={`flex gap-1 py-1 border-b border-gray-800/50 text-xs font-mono ${rowClassName} ${expandable ? 'cursor-pointer hover:bg-gray-800/50 transition-colors duration-150' : ''} ${expandable && isExpanded ? 'bg-gray-800/80' : ''}`}
