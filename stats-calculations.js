 ...
 *
 * USAGE:
 * RUN: $env:DB_USER='postgres'; $env:DB_PASSWORD='NopoONpelle31?'; $env:DB_HOST='172.29.253.202'; $env:DB_PORT='5432'; $env:DB_NAME='mydb'; $env:DB_SSL='false'; npx ts-node stats-calculations.js [function] [--fixture-ids=id1,id2,id3] [--views]
 *
 * OPTIONS:
...
async function runCalculations() {
  try {
    ...
    -- For full recalculation (fixture_ids IS NULL): recalculates ALL fixtures from the beginning
    -- For incremental calculation (fixture_ids IS NOT NULL): loads existing ELO state and updates incrementally
    ...
    BEGIN
        -- Calculate ELOs: full recalculation for all fixtures, incremental for specific fixtures
        ...
        -- Create temporary table for team ELO ratings
        ...
        -- For incremental calculations (specific fixtures), load existing ELO ratings
        -- For full recalculation (all fixtures), start from league base ELOs
        IF fixture_ids IS NOT NULL THEN
            -- Incremental mode: Load most recent ELO ratings for all teams involved
            ...
            SELECT
                ...
                ut.league_id,
                COALESCE(
                    -- Get most recent ELO for this team from any previous fixture
                    (SELECT
                        CASE
                            WHEN f.home_team_id = ut.team_id THEN fs.elo_home
                            ELSE fs.elo_away
                        END
                     FROM football_stats fs
                     JOIN football_fixtures f ON fs.fixture_id = f.id
                     WHERE (f.home_team_id = ut.team_id OR f.away_team_id = ut.team_id)
                       AND fs.fixture_id != ANY(fixture_ids)  -- Exclude current fixtures
                       AND ((f.home_team_id = ut.team_id AND fs.elo_home IS NOT NULL) OR
                            (f.away_team_id = ut.team_id AND fs.elo_away IS NOT NULL))
                     ORDER BY f.date DESC
                     LIMIT 1),
                    -- Fallback to league base ELO
                    COALESCE(lr.base_elo, 1500)
                ) as elo_rating
            FROM (
                SELECT DISTINCT team_id, league_id FROM (
                    SELECT home_team_id as team_id, league_id FROM football_fixtures WHERE id = ANY(fixture_ids)
                    UNION
                    SELECT away_team_id as team_id, league_id FROM football_fixtures WHERE id = ANY(fixture_ids)
                ) teams
            ) ut
            LEFT JOIN league_elo_ratings lr ON ut.league_id = lr.league_id
            ON CONFLICT (team_id) DO NOTHING;
        END IF;

        -- Process fixtures in chronological order
        ...
        LOOP
            -- Get league base ELO (for fallback when team not found)
